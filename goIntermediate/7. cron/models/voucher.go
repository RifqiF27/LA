...
type Voucher struct {
	...
	PaymentMethods  JSONStringSlice `gorm:"type:jsonb" json:"payment_methods,omitempty" binding:"required" swaggertype:"array,string" example:"Credit Card"`
	...
	ApplicableAreas JSONStringSlice `gorm:"type:jsonb" json:"applicable_areas,omitempty" binding:"required" swaggertype:"array,string" example:"Jawa"`
...
// Implement the Scanner interface for reading from database
func (j *JSONStringSlice) Scan(value interface{}) error {
	// If the value is nil, set it to an empty slice
	if value == nil {
		*j = JSONStringSlice{}
		return nil
	}

	// Convert []byte (the SQL value type) to []string
	byteValue, ok := value.([]byte)
	if !ok {
		return fmt.Errorf("failed to scan JSONStringSlice: expected []byte, got %T", value)
	}

	// Unmarshal the JSON into a []string
	return json.Unmarshal(byteValue, j)
}

// Implement the Valuer interface for writing to database
