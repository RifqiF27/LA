...
func (ctrl *OrderController) WebSocketOrders(c *gin.Context) {
	...
	for {
		result, err := ctrl.Service.Order.CountOrders()
		if err != nil {
			return 
		}
		
		// Simulasi result acak
		data := fmt.Sprintf(`{"timestamp": "%s", "value": %d}`, time.Now().Format(time.RFC3339), result)
