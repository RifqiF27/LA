...
type VoucherService interface {
	...
	GetUsedVouchers() ([]*models.History, error)
}
...
func (s *voucherService) GetUsedVouchers() ([]*models.History, error) {
	s.log.Info("Fetching used vouchers")

	histories, err := s.repo.History.FindAll()
	if err != nil {
		s.log.Error("Error fetching used vouchers", zap.Error(err))
		return nil, err
	}
